...
from eth_web3._utils.ens import (
    is_ens_name,
)
...
# is not automatically available on a eth_web3 instance. To use the `PM` module,
# please enable the package management API on an individual eth_web3 instance.
#
...
class ERC1319Registry(ABC):
    ...
    accessed directly, but rather through the methods on ``eth_web3.pm``.
    ...
    with `eth_web3.pm.PM`. Any implementation specific logic should be
    ...
    def __init__(self, address: Address, w3: Web3) -> None:
        """
        Initializes the class with the on-chain address of the registry, and a eth_web3
        instance connected to the chain where the registry can be found.

        Must set the following properties...

        * ``self.registry``: A `eth_web3.contract` instance of the target registry.
        * ``self.address``: The address of the target registry.
        * ``self.w3``: The *eth_web3* instance connected to the chain where the
...
class PM(Module):
    ...
    def set_registry(self, address: Union[Address, ChecksumAddress, ENS]) -> None:
        """
        Sets the current registry used in ``eth_web3.pm`` functions that read/write
    ...
    def deploy_and_set_registry(self) -> ChecksumAddress:
        ...
        ``eth_web3.pm.registry``.

        To tie your registry to an ENS name, use eth_web3's ENS module, ie.
    ...
    def release_package(
        ...
        Requires ``eth_web3.PM`` to have a registry set. Requires
        ``eth_web3.eth.default_account`` to be the registry owner.
    ...
    def _validate_set_registry(self) -> None:
        try:
            ...
            raise PMError(
                "eth_web3.pm does not have a set registry. "
                "Please set registry with either: "
                "eth_web3.pm.set_registry(address) or "
                "eth_web3.pm.deploy_and_set_registry()"
            )
        if not isinstance(self.registry, ERC1319Registry):
            raise PMError(
                "eth_web3.pm requires an instance of a subclass of ERC1319Registry "
                "to be set as the eth_web3.pm.registry attribute. Instead found: "
    ...
    def _validate_set_ens(self) -> None:
        if not self.w3:
            raise InvalidAddress(
                "Could not look up ENS address because no eth_web3 " "connection available"
            )
        elif not self.w3.ens:
            raise InvalidAddress(
                "Could not look up ENS address because eth_web3.ens is " "set to None"
            )
