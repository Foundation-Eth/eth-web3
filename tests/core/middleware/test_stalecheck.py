...
from eth_web3.middleware.stalecheck import (
    StaleBlockchain,
    _is_fresh,
...
def test_stalecheck_pass(request_middleware):
    with patch("eth_web3.middleware.stalecheck._is_fresh", return_value=True):
...
def test_stalecheck_fail(request_middleware, now):
    with patch("eth_web3.middleware.stalecheck._is_fresh", return_value=False):
...
def test_stalecheck_ignores_get_by_block_methods(request_middleware, rpc_method):
    ...
    with patch("eth_web3.middleware.stalecheck._is_fresh", side_effect=[False, True]):
...
def test_stalecheck_calls_is_fresh_with_empty_cache(
    ...
    with patch(
        "eth_web3.middleware.stalecheck._is_fresh", side_effect=[False, True]
    ) as fresh_spy:
        block = object()
...
def test_stalecheck_adds_block_to_cache(request_middleware, allowable_delay):
    with patch(
        "eth_web3.middleware.stalecheck._is_fresh", side_effect=[False, True, True]
    ) as fresh_spy:
        block = object()
        request_middleware.web3.eth.get_block.return_value = block
